...
class NewsController extends CommonController
{
     ...
     *                      "created_at": "2017-12-07 14:27:17",
     *                      "updated_at": "2017-12-07 15:14:59",
     *                      "imgs": [ // 新闻图片
    ...
    public function lists(Request $request)
    {
        ...
        if(!empty($val['industry_code'])) {
            ...
            $contentnews = DB::table('xc_content')
                               ...
                               ->where(function($query){
                                    $query->
                                    where(['position'=>'2', ['keyword_code','!=',''], ['news_id','!=','']])->
                                    orWhere(['position'=>'3', ['keyword_code','!=',''], ['news_id','!=','']]);
                                })
                               ->select(['news_id'])
                               ->groupBy('news_id')
            ...
            $contentnews = DB::table('xc_content')
                               ...
                               ->where(function($query){
                                    $query->
                                    where(['position'=>'1', ['keyword_code','!=',''], ['news_id','!=',''] ])->
                                    orWhere(['position'=>'3', ['keyword_code','!=',''], ['news_id','!=',''] ]);
                                })
                               ->select(['news_id'])
                               ->groupBy('news_id')
        ...
        if($contentnews->toArray()['data']) {
            ...
            if(!empty($val['industry_code'])) {
                $returninfo['seo'] = DB::table('sys_industry')->where('code',$val['industry_code'])->select(['name','keywords','description'])->first();
            }
            $returninfo['data'] = $datainfo;
            return $this->responseFromat(200,$returninfo);
        } else {
    ...
    public function infos(Request $request)
    {
        ...
        if ($validator->fails()) return $this->responseFromat(501,$validator);
        $newsData = DB::table('draft')->leftJoin('sys_user','draft.author','=','sys_user.id')->where(['draft.id'=>$val['draft_id'], 'draft.deleted_at'=>null])->select(['draft.id','title','subtitle','name as author','email'])->first();
        $newskeywords = DB::table('draft_keyword')->leftJoin('sys_keyword','draft_keyword.keyword_code','=','sys_keyword.code')->where(['draft_id'=>$val['draft_id'], 'draft_keyword.deleted_at'=>null])->orderBy('draft_keyword.order')->select(['keyword_code','name','name_en'])->get();
        $newsindustrys = DB::table('draft_industry')->leftJoin('sys_industry','draft_industry.industry_code','=','sys_industry.code')->where(['draft_id'=>$val['draft_id'], 'draft_industry.deleted_at'=>null])->orderBy('draft_industry.order')->select(['industry_code','name'])->get();
        $newscontentData = DB::table('draft_body')->where(['draft_id'=>$val['draft_id'], 'draft_body.deleted_at'=>null])->orderBy('element_order')->select(['element_type','element_content','element_id as id'])->get();
        ...
        foreach ($newscontentData as $key => $va) {
            ...
            if($va->element_type == 'text') {
                $newstext .= $va->element_content;
            }
            ...
            if($va->element_type == 'picture') {
                ...
                if($picturedata) {
                    ...
                    $va->original_path = $picturedata->original_path."!width900";
                }
            }
        ...
        $keyData = DB::table('draft_keyword')->leftJoin('sys_keyword','draft_keyword.keyword_code','=','sys_keyword.code')->where(['draft_id'=>$val['draft_id'], 'draft_keyword.deleted_at'=>null])->select(['name','name_en','code','oneword','desc'])->get();
        if($newskeywords->toArray() && $newsindustrys->toArray()) {
            $seokeywords = $newskeywords[0]->name.'，'.$newskeywords[0]->name_en.'，'.$newsindustrys[0]->name;
        } else if($newskeywords->toArray() && !$newsindustrys->toArray()) {
            $seokeywords = $newskeywords[0]->name.'，'.$newskeywords[0]->name_en;
        } else if(!$newskeywords->toArray() && $newsindustrys->toArray()) {
            $seokeywords = $newsindustrys[0]->name;
        }
        $data['seo'] = [ 'title'=>$newsData->title.'，'.$newsData->subtitle, 'keywords'=>$seokeywords, 'description'=>mb_substr( strip_tags($newstext), 0, 150, 'utf-8' ).'……',  ];
