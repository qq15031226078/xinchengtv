...
class HomeController extends CommonController
{
    ...
    public function slide(Request $request)
    {
        ...
        do{
            ...
            foreach ($res['slide'] as $key => $value) {
                $res['slide'][$key]->photo = $value->photo.'!1920.900';
            }
            $code = 200;
            $response = $res;
            $message = NULL;
    ...
    public function program()
    {
        ...
        do{
            ...
            $res['business'] = DB::table('xc_program as p')->join('sys_type as t','p.type_code_website','=','t.code','left')->select('p.id','p.program_type','p.title','p.subtitle','p.type_code_website','t.name as type_name','p.issue','p.thumb_path','p.link')->where(['p.program_type'=>1,'p.is_published'=>1,'p.deleted_at'=>NULL])->limit(2)->orderBy('p.id','desc')->get();

            $res['pixel']    = DB::table('xc_program as p')->join('sys_type as t','p.type_code_website','=','t.code','left')->select('p.id','p.program_type','p.title','p.subtitle','p.type_code_website','t.name as type_name','p.issue','p.thumb_path','p.link')->where(['p.program_type'=>2,'p.is_published'=>1,'p.deleted_at'=>NULL])->limit(4)->orderBy('p.id','desc')->get();
