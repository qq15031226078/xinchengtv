...
class CommonController extends Controller
{
    ...
    public function draftInfo($draftid) {
        ...
        $data = [];
        $keywordcode = DB::table('draft_keyword')->where(['draft_id'=>$draftid, 'deleted_at'=>null])->orderBy('draft_keyword.order','asc')->select(['keyword_code'])->first()->keyword_code;
        if($keywordcode) {
            $data['keyword_code'] = $keywordcode;
        }
        $draftdata = DB::table('draft')->where(['id'=>$draftid, 'deleted_at'=>null])->select(['id as draft_id','title','subtitle','created_at','updated_at'])->first();
        $draftimg = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'picture', 'draft_body.deleted_at'=>null])->leftJoin('xc_picture','draft_body.element_id','=','xc_picture.id')->select(['original_path'])->limit(3)->get();
        if($draftdata) {
            $data['draft_id'] = $draftdata->draft_id;
            $data['title'] = $draftdata->title;
            $data['subtitle'] = $draftdata->subtitle;
            $data['created_at'] = $draftdata->created_at;
            $data['updated_at'] = empty($draftdata->updated_at) ? $draftdata->created_at : $draftdata->updated_at;
            if(count($draftimg) > 0) {
                foreach($draftimg as $key => $va) {
                    $data['imgs'][] = $va->original_path; 
                }
            }
        }
        $draftgallery = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'gallery', 'draft_body.deleted_at'=>null])->leftJoin('xc_gallery','draft_body.element_id','=','xc_gallery.id')->select(['xc_gallery.id','title'])->limit(1)->first();
        ...
        $draftintro = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'text', 'deleted_at'=>null])->limit(3)->select(['element_content'])->get();
        if(count($draftintro) > 0) {
            $draftintros = '';
            foreach($draftintro as $val) {
                $draftintros .= strip_tags($val->element_content);
            }
            $data['intro'] = $this->transformationtwo($draftintros);
        } else {
            $data['intro'] = null;
        }
        // $draftintro = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'intro'])->select(['element_content'])->get();
        // if(count($draftintro) > 0) {
        //     foreach($draftintro as $val) {
    ...
    public function draftbody($draftid) {
        if(empty($draftid)) return null;
        $data = [];
        $keywordcode = DB::table('draft_keyword')->where('draft_id',$draftid)->orderBy('draft_keyword.order','asc')->select(['keyword_code'])->first()->keyword_code;
        if($keywordcode) {
            $data['keyword_code'] = $keywordcode;
        }
        $draftdata = DB::table('draft')->where('id',$draftid)->select(['id as draft_id','title','subtitle'])->first();
        $draftimg = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'picture'])->leftJoin('xc_picture','draft_body.element_id','=','xc_picture.id')->select(['thumb_path'])->limit(3)->get();
        if($draftdata) {
            $data['draft_id'] = $draftdata->draft_id;
            $data['title'] = $draftdata->title;
            $data['subtitle'] = $draftdata->subtitle;
        ...
        $draftgallery = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'gallery'])->leftJoin('xc_gallery','draft_body.element_id','=','xc_gallery.id')->select(['xc_gallery.id','title'])->limit(1)->get();
        if(count($draftgallery) > 0) {
            $data['gallerys'] = $draftgallery;
    ...
    public function keyinfo($code)
    {
        ...
        if($keyinfo) {
            ...
            $keywords['logo_path'] = $keyinfo->logo_path."!300x300";
            $keywords['head_path'] = $keyinfo->head_path;
            $product = DB::table('xc_core_value')->where(['keyword_code'=>$code, 'type'=>'p', 'deleted_at'=>null])->select(['id as core_id','name'])->get();
            $tech = DB::table('xc_core_value')->where(['keyword_code'=>$code, 'type'=>'t', 'deleted_at'=>null])->select(['id as core_id','name'])->get();
            $project = DB::table('xc_core_value')->where(['keyword_code'=>$code, 'type'=>'s', 'deleted_at'=>null])->select(['id as core_id','name'])->get();
