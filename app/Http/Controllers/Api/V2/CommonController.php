...
class CommonController extends Controller
{
    ...
    public function industryFullname($code){
        ...
        if($industryinfo == NULL)    return NULL;
        if(($industryinfo->parent) == ''){
    ...
    public function keywordInfo($code){
        ...
        $keywordinfo->logo_path = $keywordinfo->logo_path."!300x300";
    ...
    public function blockInfo($keyword_code){
    	$this->getObject();
        $data = $this->keywordblock->getKeywordBlockAll(['keyword_code'=>$keyword_code, 'deleted_at'=>null],['id','title']);
        foreach ($data as $k => $v) {
            $content = $this->keywordblockinfo->getKeyWordBlockContents(['block_id'=>$v->id, 'deleted_at'=>null],'content');
            if(count($content)) {
                $v->contents = $content;
            } else {
                unset($data[$k]);
    ...
    public function draftInfo($draftid) {
        ...
        $data = [];
        $keywordData = DB::table('draft_keyword')->leftJoin('sys_keyword','draft_keyword.keyword_code','=','sys_keyword.code')->where('draft_id',$draftid)->orderBy('draft_keyword.order','asc')->select(['keyword_code','name as keyword_name','logo_path'])->first();
        if($keywordData) {
            $data['keyword_code'] = $keywordData->keyword_code;
            $data['keyword_name'] = $keywordData->keyword_name;
            $data['logo_path']    = $keywordData->logo_path;
        }
        $industrydata = DB::table('draft_industry')->where('draft_id',$draftid)->orderBy('order','asc')->select(['industry_code'])->first();
        ...
        $draftdata = DB::table('draft')->where('id',$draftid)->select(['id as draft_id','title','subtitle','created_at'])->first();
        $draftimg = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'picture'])->leftJoin('xc_picture','draft_body.element_id','=','xc_picture.id')->select(['original_path'])->limit(2)->get();
        if($draftdata) {
            $data['draft_id'] = $draftdata->draft_id;
            $data['title'] = $draftdata->title;
            $data['subtitle'] = $draftdata->subtitle;
            $data['addtime'] = date('Y.m.d', strtotime($draftdata->created_at));
            if(count($draftimg) > 0) {
                foreach($draftimg as $key => $va) {
                    $data['imgs'][] = $va->original_path.'!app540'; 
                }
            }
        }else{
            $data['addtime'] = '';
        }
        $draftintro = DB::table('draft_body')->where(['draft_id'=>$draftid, 'element_type'=>'text'])->select(['element_content'])->limit(2)->get();
        $draftintros = '';
