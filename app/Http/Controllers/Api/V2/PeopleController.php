...
class PeopleController extends CommonController
{
    ...
    {
        $val = $req->all();
        if (empty($val['keyword_code'])) return $this->responseFromat(4300,[],'缺少参数！');
        $data['keywords'] = $this->keywordInfo($val['keyword_code']);
        $data['people'] = DB::table('xc_management')
                                ->where(['keyword_code'=>$val['keyword_code'], 'xc_management.deleted_at'=>null])
        ...
        foreach($data['people'] as $key => $val) {
            $data['people'][$key]->head_path = $val->logo_path."!draft650";
    ...
    /**
     * @api {get} /v2/keyword/peopleinfo 人物详情 
     * @apiDescription 人物详情------------------------------------王磊
     * @apiGroup Content
     * @apiPermission 需要验证
     * @apiParam {string}   keyword_code    关键词code（NvXUKMnqdD2FVol6，rwTxFK2VJZDqMcmI，X2IRnGh0fTQscvPZ，c8moPDQfmWAwG48w，u1KD6oY7xuOOFoqN，0TpVeXtynhwTl0fR，wpOpVjZkvAOVtIaS，G97PnKRFlCsueJeH，me01RHZjFW7GGibs）
     * @apiVersion 0.0.1
     * @apiSuccessExample {json} Success-Response:
     *   HTTP/1.1 201 Created
     *      {
     ...
     *      }
     */
    // public function keywordpeopleinfo(Request $req)
    // {
    //     $val = $req->all();
    //     if (empty($val['keyword_code'])) return $this->responseFromat(4300,[],'缺少参数！');
    //     $data['keywords'] = $this->keywordInfo($val['keyword_code']);
    //     $data['gallerys'] = $this->keywordGallery($val['keyword_code']);
    //     $data['block'] = DB::table('data_keyword_block')->where('keyword_code',$val['keyword_code'])->select(['id as block_id','title'])->get();
    //     foreach($data['block'] as $key => $va) {
    //         $picture = DB::table('data_keyword_block_info')->where('block_id',$va->block_id)->get(['content']);
    //         $data['block'][$key]->info = $picture;
    //     }
    //     return $this->responseFromat(200,$data);
    // }
    public function keywordpeopleinfo($code)
    {
        ...
        $block = DB::table('data_keyword_block')->where(['keyword_code'=>$code, 'deleted_at'=>null])->select(['id as block_id','title'])->get();
        foreach($block as $key => $va) {
            $picture = DB::table('data_keyword_block_info')->where(['block_id'=>$va->block_id, 'deleted_at'=>null])->get(['content']);
