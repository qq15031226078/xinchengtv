...
class NewsController extends CommonController
{
     ...
     * @apiParam {string}	keyword_code	关键词code(可选参数)
     * @apiVersion 0.0.1
     ...
     */
     ...
     {
    	$val = $req->all();
    	if (empty($val['draft_id']) ) return $this->responseFromat(4300,[],'缺少参数！');
        if(empty($val['keyword_code'])) {
            $keyword_cod = DB::table('draft_keyword')->where(['draft_id'=>$val['draft_id'], 'deleted_at'=>null])->select(['keyword_code'])->first();
            if(empty($keyword_cod)) return $this->responseFromat(4300,[],'关键词不存在！');
            $keyword_code = $keyword_cod->keyword_code;
        } else {
            $keyword_code = $val['keyword_code'];
        }
        $keywordData = $this->keywordInfo($keyword_code);
        $newsData = DB::table('draft')->leftjoin('sys_user','draft.author','=','sys_user.id')->where(['draft.id'=>$val['draft_id'], 'draft.deleted_at'=>null])->select(['draft.id','title','subtitle','name as author','email','draft.created_at'])->first();
        if(!$newsData) return $this->responseFromat(4300,[],'新闻不存在！');
        $newsData->created_at = date('Y-m-d', strtotime($newsData->created_at));
    	$newscontentData = DB::table('draft_body')->where(['draft_id'=>$val['draft_id'], 'deleted_at'=>null])->orderBy('element_order')->select(['element_type','element_content','element_id as id'])->get();
        $types = ['picture', 'gallery', 'video'];
        $texttype = ['block', 'industry_bginfo', 'product', 'project', 'base_desc'];
        $newsintro = [];
    	foreach ($newscontentData as $key => $va) {
            ...
            if(in_array($va->element_type, $texttype)) {
                $newscontentData[$key]->element_type = 'text';
            }
            ...
            if($va->element_type == 'picture') {
                ...
                if($picturedata) {
                    ...
                    $va->thumb_path = $picturedata->original_path."!width900";
     ...
     */
     public function getnewsthumb($draft_id)
     {
        ...
        $thumb_path = DB::table('draft')->where(['id'=>$draft_id])->select(['thumb_path'])->first();
