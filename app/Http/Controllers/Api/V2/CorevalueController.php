...
class CorevalueController extends CommonController
{
    ...
    public function getContentItem(Request $request)
    {
        ...
        if(in_array($request->get('type'), ['p','t','s'])) {
            $response = DB::table($tableParams[$request->get('type')])
                ->where(['b.deleted_at' => NULL])
                ->whereIn('a.industry_code', $where)
                ->where($whereParmaConfig[0], $whereParmaConfig[1], $whereParmaConfig[2])
                ->join('xc_core_value as b', $whereParmaConfig[0], '=', 'b.id', 'left')
                ->select($select[$request->get('type')])
            ...
            $response = DB::table($tableParams[$request->get('type')])
                ...
                ->where($whereParmaConfig[0], $whereParmaConfig[1], $whereParmaConfig[2])
                ->join('sys_keyword as c', 'c.code', '=', 'a.keyword_code', 'left')
                ->select($select[$request->get('type')])
                ->paginate($request->get('per_page', 5));
        }
    ...
    public function KeywordIntro(Request $request){
        ...
        $keywordsItem = DB::table('draft_keyword')->orderBy('id','desc')->limit(3)
