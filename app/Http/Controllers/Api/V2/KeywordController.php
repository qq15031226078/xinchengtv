...
class KeywordController extends CommonController
{
    ...
    public function CoreValue ($keyword_code,$type){
    	$data = DB::table('xc_core_value')->where(['keyword_code'=>$keyword_code,'type'=>$type])->orderBy('order','asc')->orderBy('created_at','desc')->select('id','name','name_en','thumb_path')->get();
    	foreach ($data as $k => $v) {
            $content = DB::table('data_core_record')->where(['core_id'=>$v->id])->orderBy('order','asc')->orderBy('created_at','asc')->pluck('content');
    		if(count($content)) {
                $v->content = $content;
            } else {
                unset($data[$k]);
    ...
    public function KeywordCore($core_id){
    	...
    	$core['contents'] = DB::table('data_core_record')->where(['core_id'=>$core['id']])->orderBy('order','asc')->orderBy('created_at','asc')->pluck('content');
     ...
     * @apiSuccessExample {json} Success-Response:
{
    ...
    "response": [
        {
            ...
            "intro": "2015年年初，美国00后最喜欢的照片社交分享应用Snapchat完成了新一轮高达4.86亿美元的融资，市场估值为150……"
        }
    ]
}
     */
    public function search(Request $request){
        ...
        do {
            ...
            // 先找新闻
            $news_id_by_draft = json_decode(DB::table('db_writing.draft')->select(['id as news_id'])->where('title','like','%'.$search.'%')->orWhere('subtitle','like','%'.$search.'%')->get(),TRUE);
            ...
            $news_id_by_keyword = json_decode(DB::table('db_writing.draft_keyword')->select(['draft_id as news_id'])->whereIn('keyword_code',$keyword_codes)->get(),TRUE);
